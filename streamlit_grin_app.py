...
def ensure_resources():
    ...
    beams_dir = os.path.join(here, "beams")
    d2t_dir   = os.path.join(here, "d2t")
    os.makedirs(beams_dir, exist_ok=True)
    os.makedirs(d2t_dir, exist_ok=True)
...
st.set_page_config(page_title="GRIN Lens Builder", layout="wide")
st.title("GRIN Lens Builder")
st.caption("Build Luneburg sphere/cylinder or custom GRIN. Visualize ε, thickness, and lattice. Export thickness grid.")
...
with st.sidebar:
    ...
    shape = st.selectbox("Profile", ["luneburg_sphere", "luneburg_cylinder", "custom_grid"], index=0)
    ...
    R = st.number_input("Radius R [mm] (sphere/cylinder)", min_value=0.0, max_value=2000.0, value=60.0, step=1.0)
    ...
    try:
        ...
        if shape == "luneburg_sphere":
            ...
            if custom_expr is not None:
                ...
                ln.make(shape=shape, out_shape=(int(nx), int(ny), int(nz)), X=X, Y=Y, Z=Z, custom_eps_func=eps_func)
            else:
                ln.make(shape=shape, out_shape=(int(nx), int(ny), int(nz)), X=X, Y=Y, Z=Z)

        # Persist results
        st.session_state["grin_result"] = {
            "eps_grid": ln.eps_grid,
            "t_grid": ln.thickness_grid,
            "lattice_vol": ln.lattice,  # shape (300,300,Dz_lat) with Dz_lat ~ 100
...
if "grin_result" in st.session_state:
    ...
    lat_slice = lattice[:, :, z_idx]

    # Fixed color limits across slices
    ...
    if st.button("Write file"):
        try:
            ...
            if ln is None:
                ...
                ln.write_thickness(tmp_path, origin=(0,0,0))
